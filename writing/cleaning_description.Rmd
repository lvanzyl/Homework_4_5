...
- alc_res: measured alcohol blood level
- lag_hrs: how many hours after the accident the person died
- lag_mins: how many minutes after the number of lag_hrs a person died
...
Hawaii, Illinois, New Hampshire, Rhode Island, and West Virginia. These were states that performed toxicological testing on more than 80% of their fatally injured drivers. The state column was then removed, since none of the analysis was done by individual state in the Brady and Li study.

7. First mutate the  sex column so that any entries with a value of 9 and actually NA values, but that if the entry is not equal to 9, just keep the original entry. After this, say that the entries in this column correspond to two levels, 1 and 2, and then label these two levels as "Male" and "Female" accordingly.
